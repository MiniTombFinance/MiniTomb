...
/* STAKING MECHANICS */
   ...
   function clearUber(uint256[] calldata _users) public onlyOwner {
    ...
    function setLand(address _landContract) public onlyOwner() {
    ...
    function setTaxRate5(uint256 _newTax) public onlyOwner() {
      taxRate5 = _newTax;
    }

    function setTaxAmt1(uint256 _newTaxAmt) public onlyOwner() {
      taxAmt1 = _newTaxAmt;
    }

    function setTaxAmt2(uint256 _newTaxAmt) public onlyOwner() {
      taxAmt2 = _newTaxAmt;
    }

     function setTaxAmt3(uint256 _newTaxAmt) public onlyOwner() {
      taxAmt3 = _newTaxAmt;
    }

     function setTaxAmt4(uint256 _newTaxAmt) public onlyOwner() {
      taxAmt4 = _newTaxAmt;
    }
    ...
    function claimRewards(uint256[] calldata tokenIds) public whenNotPaused {
      ...
      for (uint256 i; i < tokenIds.length; i++) {
       ...
       //checks if tokenId is uber which means they only pay 10% tax no matter what amount
